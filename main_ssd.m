% MAIN_SSD
% SSD kolunu çalıştırır.
% Çıktıları hem results_ssd içine hem de düzenli şekilde results_final içine kaydeder.
...
finalMatDir   = fullfile(finalDir, 'mat', 'ssd');
...
dirsToCreate = {resultsDir, finalDir, finalMatDir, finalFigDir};

for i = 1:numel(dirsToCreate)
    if ~exist(dirsToCreate{i}, 'dir')
        mkdir(dirsToCreate{i});
    end
end
...
for v = 1:length(videoDirs)
    ...
    %% TEMİZ RESULTS_FINAL İÇİNE KAYIT
    finalTrajMat = fullfile(finalMatDir, sprintf('%s_%s_ssd_traj.mat', setName, videoName));
    finalGraphMat = fullfile(finalMatDir, sprintf('%s_%s_ssd_graph.mat', setName, videoName));
    ...
    opts.fileName = sprintf('%s_%s_ssd_trajectory.png', setName, videoName);
    ...
    fprintf("PNG kaydedildi: %s\n", fullfile(finalFigDir, opts.fileName));
